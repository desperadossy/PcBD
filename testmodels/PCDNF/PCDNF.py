...
class ConsistentPointSelect(nn.Module):
    ...
    def forward(self, pointfea, normalfea, point, normal):
        '''
...
class PCDNF(nn.Module):
    ...
    def forward(self, x, ):
        ...
        _, _, _, grouped_xyz = sample_and_group_knn(x, points=None, npoint=x.size(-1), k=64) # [B, 3, N, k], [B, N, k]
        normal = knn_cal_axis(grouped_xyz.permute(0,2,3,1).contiguous())
