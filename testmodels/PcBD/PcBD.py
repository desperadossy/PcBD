...
class BoundDetect(nn.Module):
    ...
    def forward(self, xyz, pw_feat):
        ...
        bound_xyz = score_extract(xyz, bound_label, threshold=0.6, reserve_high=True) # [B, 3, N]
...
class PcBD(nn.Module):
    def __init__(self):
        super().__init__()
        self.FE = FeatureExtractor(norm_nsample = 10, knn = 16)
        self.BD = BoundDetect(embed_dim = 128)
