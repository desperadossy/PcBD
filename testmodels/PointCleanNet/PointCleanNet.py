...
class ResSTN(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.quaternion:
            ...
            # convert quaternion to rotation matrix
            if x.is_cuda:
                trans = torch.FloatTensor(batchsize, 3, 3).to(x.device)
            else:
                trans = torch.FloatTensor(batchsize, 3, 3)
            x = batch_quat_to_rotmat(x, trans)
        return x
...
class PointCleanNet(nn.Module):
    def __init__(self, num_points=500, output_dim=1, use_point_stn=True, use_feat_stn=True, sym_op='max', get_pointfvals=False, point_tuple=1):
        super(PointCleanNet, self).__init__()
        self.num_points = num_points
...
if __name__ == '__main__':
    point = torch.rand(2, 512, 3).cuda()
    
    net = PointCleanNet(output_dim=1).cuda()
    out = net(point)
    print(out.size())