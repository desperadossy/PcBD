...
def get_gan_config(args, logger=None):
    gen_config = cfg_from_yaml_file(args.config)
    cdis_config = cfg_from_yaml_file(args.cdisconfig)
    fdis_config = cfg_from_yaml_file(args.fdisconfig)
    if not args.resume and args.local_rank == 0:
        save_experiment_config(args, gen_config, logger)
    return gen_config, cdis_config, fdis_config
